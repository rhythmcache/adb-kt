...
class AdbRescue(private val connection: AdbConnection) {
    ...
    fun install(file: File, blockSize: Int = AdbSideload.DEFAULT_BLOCK_SIZE): Flow<SideloadProgress> =
        install(RandomAccessSource.of(file), blockSize)

    /**
     * Install a rescue OTA package over the ADB rescue service using a [RandomAccessSource].
     */
    fun install(source: RandomAccessSource, blockSize: Int = AdbSideload.DEFAULT_BLOCK_SIZE): Flow<SideloadProgress> = flow {
        ...
        source.use {
            try {
                ...
                while (true) {
                    ...
                    val blockNum = reqStr.toLongOrNull()
